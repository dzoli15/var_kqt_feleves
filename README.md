...
|	Az 1114 nem azt jelenti, hogy 1114 teljesen különböző, logikus főcsapás van. Azt jelenti, hogy ha a robot minden egyes elágazásnál dönthetne, és minden kis hurkot megkerülhetne így is meg úgy is, akkor 1114 különböző kombinációban tudna eljutni a kéktől a zöldig anélkül, hogy önmagába harapna.

futtatás:
 - első lépés:		terminal 1:	colcon build --packages-select var_kqt_feleves
 - második lépés:	terminal 2:	source ~/ros2_ws/install/setup.bash
					rviz2
 - harmadik lépés:	terminal 3:	source ~/ros2_ws/install/setup.bash
			{Automatikus}	ros2 run var_kqt_feleves pathfinder_node
			[Az RViz-ben 10 másodpercenként frissül a kép, a terminálban pedig pörögnek a logok.]

			{Manuális mód}	ros2 run var_kqt_feleves pathfinder_node --ros-args -p automatic_mode:=false
			terminal 4:	source ~/ros2_ws/install/setup.bash
